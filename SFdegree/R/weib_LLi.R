#' @title Likelihood function of the discretized Weibull distribution
#'
#' @description Computes the likelihood of the discretized Weibull model with a given `kmin`
#'
...
#' @param params a list object holding the `a` and `b` parameter for the discretized Weibull distribution
...
#' Each element is the likelihood value of the data evaluated at the given parameter values (`a`, `b`) and right-tail cutoff `kmin`.
#'
#' @export
#'
...
#' weib_LLi(k = c(1,2,3,4,5), kmin = 1, params = list(a = 1.1, b = 0.5))
#'
...
#' weib_LLi(k = c(2,1,3,5,4), kmin = 3, params = list(a = 1.1, b = 0.5))

weib_LLi = function(k, kmin, params, log = T){
  ...
  # log-likelihood
  if(log){
    log_Li = (kmin/b)^a - (k/b)^a + log(1 - exp((k/b)^a - ((k+1)/b)^a))
    return(log_Li)
  }

  # likelihood
  if(!log){
    Li = exp((kmin/b)^a) / exp((k/b)^a) * (1 - exp((k/b)^a - ((k+1)/b)^a))
    return(Li)
  }
}
