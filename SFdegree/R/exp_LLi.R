#' @title Likelihood function of the discrete exponential distribution
#'
...
#' @param params a list object holding the lambda parameter for the discrete exponential distribution
...
#' Each element is the likelihood value of the data evaluated at the given parameter `lambda` and right-tail cutoff `kmin`.
#'
#' @export
#'
...
#' exp_LLi(k = c(1,2,3,4,5), kmin = 1, params = list(lambda = 2.5))
#'
...
#' exp_LLi(k = c(2,1,3,5,4), kmin = 3, params = list(lambda = 2.5))

exp_LLi = function(k, kmin, params, log = T){
  ...
  # log-likelihood
  if(log){
    log_Li = log(1 - exp(-lambda)) + lambda*(kmin-k)
    return(log_Li)
  }

  # likelihood
  if(!log){
    Li = (1 - exp(-lambda)) * exp(lambda*(kmin-k))
    return(Li)
  }
}
