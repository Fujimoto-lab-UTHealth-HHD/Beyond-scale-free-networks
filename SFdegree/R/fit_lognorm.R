#' @title Fits the Discretized Log-Normal Model
#'
#' @description Standard numerical optimization routine to obtain the maximum likelihood estimate for the discrete log-normal model.
#'
...
#' @export
#'
#' @note For the discrete exponential model, the Nelder-Mead method is used for numerical optimization
#'
#' @examples
#' # a data vector
#' set.seed(1)
...
#' my_opt = fit_lognorm(
#'   k = my_k, kmin = my_kmin,
#'   inits = c(mu = 0, log.sigma = 1)
#' )

fit_lognorm = function(k, kmin, inits = c(mu = 0, log.sigma = 1), opt.controls = list(fnscale = -sum(k >= kmin), reltol = 1e-16, maxit = 1e5)){
  # subset
  k.orig = k
  k = k[k >= kmin]
  ...
  opt = optim(
    par = inits,
    fn = function(par){
      LLi = lognorm_LLi(k = k, kmin = kmin, params = list(mu = unname(par['mu']), sigma = unname(exp(par['log.sigma']))),
                        log = TRUE)
  ...
  # output
  out = list(
    par = c(mu = unname(opt$par['mu']), sigma = unname(exp(opt$par['log.sigma']))),
